...
//!
//! This example validates that Kokoro-82M works through the standard
...
//! - voices.bin voice embeddings (NPZ format)
//! - misaki/ dictionary files (us_gold.json, us_silver.json)
//!
//! Usage:
//!   cargo run --example tts_kokoro
//!   cargo run --example tts_kokoro "Hello, how are you today?"
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("═══════════════════════════════════════════════════════");
    println!("  Kokoro-82M TTS using Xybrid Execution System");
    println!("═══════════════════════════════════════════════════════");
    println!();
    ...
    let text = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "Hello world, this is Kokoro speaking.".to_string());
    ...
    if let Some(voices) = metadata.metadata.get("voice_names") {
        if let Some(voices_array) = voices.as_array() {
            println!("🎤 Available voices ({}):", voices_array.len());
            for (i, voice) in voices_array.iter().take(5).enumerate() {
                println!("     {}. {}", i, voice.as_str().unwrap_or("?"));
            }
            if voices_array.len() > 5 {
                println!("     ... and {} more", voices_array.len() - 5);
            }
            println!();
        }
    }

    // Create TemplateExecutor (same pattern as other TTS models)
    ...
    match &output_envelope.kind {
        EnvelopeKind::Audio(audio_bytes) => {
            ...
            // Save to WAV file
            let output_path = PathBuf::from("tts_kokoro_output.wav");
            save_wav(&output_path, audio_bytes, sample_rate)?;
            println!("💾 Saved to: {}", output_path.display());
            println!();
            println!("🎵 Play the output:");
            println!("   afplay tts_kokoro_output.wav   # macOS");
            println!("   aplay tts_kokoro_output.wav    # Linux");
        }
    ...
    println!("═══════════════════════════════════════════════════════");
    println!();
    println!("✅ Kokoro-82M TTS working through Xybrid execution system!");
    println!();
    println!("🎯 This validates:");
    println!("   • model_metadata.json phonemization config is correct");
    println!("   • NPZ voice embedding loading works");
    println!("   • MisakiDictionary/EspeakNG phonemization integration");
    println!("   • TemplateExecutor handles Kokoro TTS models");
