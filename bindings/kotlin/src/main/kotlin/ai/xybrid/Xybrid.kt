...
object Envelope {
     ...
     * formats throw [IllegalArgumentException]); the bytes themselves are
     * decode-validated on the Rust side at run time (surfacing as a
     * [XybridError.InvalidImage] for bad or oversized input).
    ...
    fun userMessage(text: String, images: List<XybridEnvelope> = emptyList()): XybridEnvelope {
        require(images.all { it.kind is XybridEnvelopeKind.Image }) {
     ...
     * with [IllegalArgumentException] rather than deferring to a run-time
     * [XybridError.InvalidImage].
     */
