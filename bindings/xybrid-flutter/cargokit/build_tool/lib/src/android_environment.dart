...
class AndroidEnvironment {
  ...
  Future<Map<String, String>> buildEnvironment() async {
    ...
    // Allow overriding the minimum SDK version for Rust builds via environment variable.
    // This is useful when Rust dependencies require newer Android APIs (e.g., aws-lc-sys
    // requires API 28+ for getentropy). Set CARGOKIT_RUST_MIN_SDK_VERSION=28 to override.
    // See: https://github.com/tauri-apps/tauri/issues/11964
    final rustMinSdkOverride = Platform.environment['CARGOKIT_RUST_MIN_SDK_VERSION'];
    final effectiveMinSdk = rustMinSdkOverride != null
        ? int.parse(rustMinSdkOverride)
        : this.minSdkVersion;
    final minSdkVersion =
        math.max(target.androidMinSdkVersion!, effectiveMinSdk);
  ...
  String _libGccWorkaround(String buildDir, Version ndkVersion) {
    ...
    // Add FP16 target feature for ARM64 Android targets.
    // This is required for gemm-f16/gemm-common crates (used by Candle/mistral.rs)
    // which use ARM NEON FP16 instructions that require the fullfp16 feature.
    // Without this, builds fail with "instruction requires: fullfp16"
    ...
    if (target.rust == 'aarch64-linux-android') {
      rustFlags = '$rustFlags\x1f-Ctarget-feature=+fp16';
    }
