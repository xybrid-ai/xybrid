...
The batch, context, and streaming APIs accept the standard .NET
`CancellationToken`. Cancelling it signals the Rust generation loop and the
returned task completes as cancelled once the native run reaches its next
...
Task<InferenceResult> pending = model.RunAsync(
    Envelope.Text("Write a long story"),
    cancellationToken: stop.Token);

// For example, from a UI Stop action:
stop.Cancel();
...
{
    // Expected: native inference was cooperatively stopped.
}
```
