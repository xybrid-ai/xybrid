...
<Callout type="info">
The Swift SDK is in early access. The API is stable but the package is not yet published as a release. Install via Swift Package Manager from source, or use the [Flutter SDK](/docs/sdks/flutter) for cross-platform development.
</Callout>
