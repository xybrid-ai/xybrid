...
await for (final event in loader.loadWithProgress()) {
  switch (event) {
    case LoadProgress(:final percentage, :final downloadedBytes, :final totalBytes):
      print('Loading: $percentage% ($downloadedBytes / ${totalBytes ?? '?'} bytes)');
    case LoadComplete():
      print('Model ready!');
...
| 事件 | 属性 | 说明 |
|-------|-----------|-------------|
| `LoadProgress` | `progress`（0.0–1.0）、`percentage`（0–100）、`downloadedBytes`、`totalBytes`（可空） | 下载/加载进度 |
| `LoadComplete` | — | 模型已就绪 |
