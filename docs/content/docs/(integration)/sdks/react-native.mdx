...
The React Native package (`react-native-xybrid`) wraps the same Rust core as the Swift and Kotlin SDKs in a TurboModule, so the API, models and error codes match every other binding. It is in **preview**: the surface is complete and tested, but may still change before 1.0.
