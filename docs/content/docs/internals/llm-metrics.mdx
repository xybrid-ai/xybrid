...
| `tokens_in` | count | `usize` | Local LLM (llama.cpp): `tokens.len()` captured before the streaming loop so it survives mid-stream aborts. Local LLM (mistral): `Usage.prompt_tokens` from the terminal chunk. Cloud LLM: upstream `Usage.prompt_tokens` when populated. |
...
| `tokens_out` | count | `usize` | Cloud LLM only: upstream `Usage.completion_tokens`. The SDK's `extract_llm_token_counts` accepts both `tokens_out` and `tokens_generated` as aliases when hoisting to the platform-event payload top-level. |
