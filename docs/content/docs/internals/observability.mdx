...
| Field | Description |
|-------|-------------|
| `ttft_ms` | Local: time to first emitted stream chunk. Cloud: gateway round-trip time. |
| `tokens_in` | Prompt token count. Local: captured before the streaming loop (survives mid-stream aborts). Cloud: from upstream `Usage.prompt_tokens`. |
| `tokens_generated` | Local LLM output token count. |
| `tokens_out` | Cloud-only: upstream `Usage.completion_tokens`. Hoisted alongside `tokens_generated`. |
