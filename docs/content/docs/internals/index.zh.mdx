...
| 组件 | 用途 |
|-----------|---------|
| `Envelope` | 音频、文本、Embedding 的通用数据容器 |
| `Orchestrator` | 将请求路由到合适的执行目标 |
| [Pipeline](/zh/docs/concepts/pipelines) | 串联多个阶段（ASR → LLM → TTS） |
| `Executor` | 通过 ONNX、llama.cpp 或 Candle 运行时执行模型 |
| `StreamSession` | 实时流式推理 |

## 数据流

1. **输入**以 `Envelope` 形式到达（音频字节、文本或 Embedding）
2. **Orchestrator** 决定执行目标（设备端、云端或回退）— 参见[混合执行](/zh/docs/concepts/hybrid-execution)
