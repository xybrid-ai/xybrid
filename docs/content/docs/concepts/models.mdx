...
- **HuggingFace repos**: `xybrid run --huggingface <repo>[:variant]` (or `fromHuggingFace` in the SDKs) downloads the model directly — GGUF, ONNX, SafeTensors, and TFLite natively, plus LiteRT-LM (`.litertlm`) in the browser SDK. When the repo lacks a `model_metadata.json`, one is auto-generated
