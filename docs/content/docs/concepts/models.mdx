...
- **HuggingFace repos**: `xybrid run --huggingface <repo>[:variant]` (or `fromHuggingFace` in the SDKs) downloads the model directly — GGUF, ONNX, and SafeTensors run natively; TFLite and LiteRT-LM (`.litertlm`) run in the browser SDK (`@xybrid/web`). When the repo lacks a `model_metadata.json`, one is auto-generated
