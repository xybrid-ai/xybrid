...
A **Pipeline** is a sequence of stages that transform data. Pipelines are defined in YAML and executed by the orchestrator, enabling you to chain models together (ASR → LLM → TTS) with automatic routing between device and cloud execution.
...
3. **Execute** - Run stages sequentially via orchestrator
