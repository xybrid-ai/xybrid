...
| Target | Behavior |
|--------|----------|
| `device` | Always run on-device (aliases: `local`, `edge`) |
| `cloud` | Always run in the cloud (aliases: `server`, `integration`) |
...
- **Device capabilities** — CPU features, memory, and accelerators (see [Observability](/docs/internals/observability))
- **Host signals** — battery level and thermal state reported by the platform (see [Host Integration](/docs/internals/host-integration))
...
You can also force a target at run time without editing the YAML:

```bash
xybrid run --config pipeline.yaml --input-audio q.wav --target device
```
...
- [Host Integration](/docs/internals/host-integration) — how battery/thermal signals reach the router
- [Observability](/docs/internals/observability) — decision methods and telemetry events
