...
UI isolate. Swift and Kotlin pair every call with an `…Async` twin
(`modelCacheStatusAsync()`, `removeCachedModelAsync(...)`) that runs off the
calling thread; Unity's calls are synchronous, so run large scans or deletions
on a worker.

There is no expiry cleanup in the bindings: the SDK does not yet persist
retention metadata, so it cannot tell an expired download from a local model
after a restart. Use targeted eviction; file age alone is not a reason to
