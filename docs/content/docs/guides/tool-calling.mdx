...
```rust
use xybrid_sdk::{ModelLoader, Tool};
...
- With a reasoning model (e.g. `lfm2.5-1.2b-thinking`), `--show-reasoning`
  prints the chain-of-thought behind each tool decision above the `⚙` lines
  and the final answer's reasoning before the answer — the clearest way to
  watch a model decide *whether* and *which* tool to use.
