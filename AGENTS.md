...
This document is the canonical Rust playbook for any AI agent working in this
repository. It bundles two upstream sources verbatim so the agent has the full
text in context (no need to fetch the web at runtime):

1. **Microsoft Pragmatic Rust Guidelines (agent edition)** —
   <https://microsoft.github.io/rust-guidelines/agents/all.txt>
2. **rust-skills SKILL.md** by leonardomso —
   <https://github.com/leonardomso/rust-skills/blob/master/SKILL.md>

Project-specific overrides and conventions for *this* repo live in
[`CLAUDE.md`](./CLAUDE.md). When the two disagree, **CLAUDE.md wins** — the
upstream guides are general best practice; CLAUDE.md captures decisions the
maintainers have already made for xybrid.

---

## How to work in this repo

The compiler and lints are your automatic reviewer. Before declaring a task
...
`just fmt` (without `-check`) writes formatting fixes. Run it before committing
to keep the `fmt` CI job green.

CI gates — see `.github/workflows/ci.yml`:

- `fmt` — `cargo fmt --all --check`
- `clippy` — `cargo clippy --workspace --all-targets -- -D warnings`
- `test` — `cargo test --workspace --features ort-download` on macOS / Linux / Windows
- Feature-matrix checks: `check-no-default-features`, `check-platform-macos`,
  `check-platform-desktop`, `clippy-llm`, `test-llm`, `test-candle`,
  `build-cli`, `api-contract`

If clippy fires on a lint that's intentionally allowed for this crate's
maturity, the lint is opted out at the crate root in `lib.rs` (see e.g.
`crates/xybrid-core/src/lib.rs`). Don't sprinkle `#[allow(...)]` at every
call-site — add it once at crate level if it's a project-wide decision, or fix
the lint locally if it's a real issue.
...
# Section 1 — Microsoft Pragmatic Rust Guidelines (agent edition)

> *Source:* <https://microsoft.github.io/rust-guidelines/agents/all.txt>
> *Embedded verbatim. License: MIT (Microsoft Corporation).*
...
# Section 2 — rust-skills SKILL.md (leonardomso)

> *Source:* <https://github.com/leonardomso/rust-skills/blob/master/SKILL.md>
> *Embedded verbatim. License: MIT.*
> *Note: linked `rules/<rule-id>.md` files are not embedded — see the upstream
> repo for full per-rule examples. The rule IDs and one-line summaries below
> are themselves a useful checklist.*
