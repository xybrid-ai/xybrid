...
mod tests {
    ...
    // span collector, so each test holds `tracing::test_lock()` — the *same*
    // process-wide mutex the executor and image tracing tests serialise on — for
    // its whole reset/measure window. `cargo test` parallelises within a binary,
    // and a private lock here would not exclude those other suites, so their
    // `reset_tracing()` could wipe these spans mid-flight.
