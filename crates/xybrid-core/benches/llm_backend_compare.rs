...
mod active {
    ...
    use xybrid_core::runtime_adapter::mlx::bench_report::{
        check_threshold, render_json, render_markdown, BenchModel, CellResult, CellStatus,
        ThresholdPolicy,
    };
    ...
    fn report_models() -> Vec<BenchModel> {
        MODELS
            .iter()
            .map(|model| BenchModel {
                display_name: model.display_name,
                mlx_fixture_id: model.mlx_fixture_id,
                gguf_fixture_id: model.gguf_fixture_id,
    ...
    fn write_reports(
        ...
        let markdown_path = dir.join("llm_backend_compare.md");
        let json_path = dir.join("llm_backend_compare.json");
        fs::write(&markdown_path, markdown)?;
        let json_body = serde_json::to_string_pretty(json)?;
    ...
    pub fn run() {
        ...
        let report_models = report_models();
        ...
        match write_reports(&markdown, &json) {
            Ok((markdown_path, json_path)) => {
        ...
        match check_threshold(&report_models, &collected, LLAMA_CPP_DECODE_PARITY_FLOOR) {
            Ok(()) => {
                let has_enforced_pairs = report_models
                    .iter()
                    .any(|model| model.threshold_policy.is_enforced());
                if has_enforced_pairs {
                    eprintln!(
                        "all enforced pairs met the {}x parity floor",
                        LLAMA_CPP_DECODE_PARITY_FLOOR
                    );
                } else {
                    eprintln!(
