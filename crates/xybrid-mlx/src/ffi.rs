...
/// Capture an mlx-c error message without unwinding across the C boundary.
///
/// # Safety
///
/// `msg` must be null or point to a valid nul-terminated C string for the
/// duration of the call. `data` is ignored.
unsafe extern "C" fn error_trampoline(msg: *const c_char, _data: *mut c_void) {
    let _ = std::panic::catch_unwind(|| {
        let message = if msg.is_null() {
            "<null MLX error message>".to_string()
        } else {
            // SAFETY: mlx-c invokes the handler with a valid nul-terminated
            // message pointer for the duration of this callback.
...
/// Install the error handler before a fallible FFI entry that may run
/// before any stream constructor (raw array allocation during weight
/// loading). Idempotent and lock-free after the first call.
pub(crate) fn ensure_error_handler() {
    ERROR_HANDLER_INSTALLED.get_or_init(|| {
        // SAFETY: `error_trampoline` has the exact handler ABI generated by
        // bindgen for mlx-c. We pass no user data, so no destructor is needed.
        unsafe {
            sys::mlx_set_error_handler(Some(error_trampoline), std::ptr::null_mut(), None);
        }
    });
}

fn take_recorded_error() -> Option<String> {
    LAST_ERROR_MESSAGE.lock().ok()?.take()
}

/// Translate an mlx-c nonzero return into an [`MlxError`].
///
/// mlx-c's convention is "0 = success, nonzero = error". The underlying
/// detail is captured by [`error_trampoline`] after [`ensure_error_handler`]
/// installs it from the stream constructors.
pub(crate) fn check_rc(rc: i32, ctx: &'static str) -> Result<(), MlxError> {
    if rc == 0 {
        Ok(())
    } else {
        let mut message = format!("mlx-c `{ctx}` returned rc={rc}");
        if let Some(recorded) = take_recorded_error().filter(|msg| !msg.is_empty()) {
            message.push_str(": ");
            message.push_str(&recorded);
        }
        Err(MlxError::Internal(message))
    }
}
...
pub(crate) unsafe fn stream_clone(src: mlx_stream) -> Result<mlx_stream, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_stream_set") {
...
pub(crate) unsafe fn array_clone(src: mlx_array) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_array_set") {
...
pub(crate) unsafe fn op_matmul(
    ...
    if let Err(err) = check_rc(rc, "mlx_matmul") {
...
pub(crate) unsafe fn op_add(
    ...
    if let Err(err) = check_rc(rc, "mlx_add") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_sub(
    ...
    if let Err(err) = check_rc(rc, "mlx_subtract") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_mul(
    ...
    if let Err(err) = check_rc(rc, "mlx_multiply") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_div(
    ...
    if let Err(err) = check_rc(rc, "mlx_divide") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_zeros(
    ...
    if let Err(err) = check_rc(rc, "mlx_zeros") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_neg(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_negative") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_square(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_square") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_erf(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_erf") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_sqrt(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_sqrt") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_tanh(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_tanh") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_softmax_axis(
    ...
    if let Err(err) = check_rc(rc, "mlx_softmax_axis") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_fast_rms_norm(
    ...
    if let Err(err) = check_rc(rc, "mlx_fast_rms_norm") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_fast_layer_norm(
    ...
    if let Err(err) = check_rc(rc, "mlx_fast_layer_norm") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_astype(
    ...
    if let Err(err) = check_rc(rc, "mlx_astype") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_contiguous(
    ...
    if let Err(err) = check_rc(rc, "mlx_contiguous") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_reshape(
    ...
    if let Err(err) = check_rc(rc, "mlx_reshape") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_transpose_axes(
    ...
    if let Err(err) = check_rc(rc, "mlx_transpose_axes") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_transpose_default(
    ...
    if let Err(err) = check_rc(rc, "mlx_transpose") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_take_axis(
    ...
    if let Err(err) = check_rc(rc, "mlx_take_axis") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_slice(
    ...
    if let Err(err) = check_rc(rc, "mlx_slice") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_slice_update(
    ...
    if let Err(err) = check_rc(rc, "mlx_slice_update") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
unsafe fn vector_array_get_owned(vec: mlx_vector_array, idx: usize) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_vector_array_get") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_concat_axis(
    ...
    if let Err(err) = check_rc(rc, "mlx_concatenate_axis") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_conv1d(
    ...
    if let Err(err) = check_rc(rc, "mlx_conv1d") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_quantized_matmul(
    ...
    if let Err(err) = check_rc(rc, "mlx_quantized_matmul") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_dequantize(
    ...
    if let Err(err) = check_rc(rc, "mlx_dequantize") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_quantize(
    ...
    if let Err(err) = check_rc(rc, "mlx_quantize") {
...
pub(crate) unsafe fn op_fast_rope(
    ...
    if let Err(err) = check_rc(rc, "mlx_fast_rope") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_sigmoid(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_sigmoid") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_exp(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_exp") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_reciprocal(a: mlx_array, s: mlx_stream) -> Result<mlx_array, MlxError> {
    ...
    if let Err(err) = check_rc(rc, "mlx_reciprocal") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_argmax_axis(
    ...
    if let Err(err) = check_rc(rc, "mlx_argmax_axis") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
pub(crate) unsafe fn op_sdpa(
    ...
    if let Err(err) = check_rc(rc, "mlx_fast_scaled_dot_product_attention") {
        let _ = sys::mlx_array_free(res);
        return Err(err);
    }
    Ok(res)
...
mod tests {
    #[test]
    fn ensure_error_handler_is_idempotent() {
        super::ensure_error_handler();
        super::ensure_error_handler();
    }
}
