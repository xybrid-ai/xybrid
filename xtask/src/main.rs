...
enum Commands {
    ...
    BuildFfi {
        ...
        /// Build `xybrid-bolt` (the BoltFFI native Unity now loads) instead of
        /// the pre-bolt `xybrid-ffi` C ABI. `--csharp` is ignored under `--bolt`
        /// (bolt's C# is generated by tools/scripts/gen_unity_bolt_csharp.py).
        #[arg(long)]
        bolt: bool,
    },
    ...
    BuildUnity {
        ...
        /// Build `xybrid-bolt` (the native Unity now loads) instead of `xybrid-ffi`.
        #[arg(long)]
        bolt: bool,
    },
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::BuildFfi {
            ...
            build_ffi(target, release, platform_preset, csharp, deploy_unity, bolt)?;
        }
        ...
        Commands::BuildUnity {
            ...
            build_unity(all_platforms, csharp, deploy, bolt)?;
        }
...
fn build_ffi(
    ...
    // Pick the native crate. Unity now loads `xybrid_bolt`; `xybrid-ffi` is the
    // pre-bolt C ABI being retired. Both expose the same `platform-*` presets, so
    // only the package name and the emitted lib file names differ.
    let crate_name = if bolt { "xybrid-bolt" } else { "xybrid-ffi" };
    let lib_stem = if bolt { "xybrid_bolt" } else { "xybrid_ffi" };
    // csbindgen C# is an xybrid-ffi concept; bolt's C# is generated separately by
    // tools/scripts/gen_unity_bolt_csharp.py, so `--csharp` is a no-op under bolt.
    let csharp = csharp && !bolt;
    ...
    if deploy_unity {
        ...
        deploy_ffi_to_unity(unity_lib_path, target.as_deref(), bolt)?;
    }
...
fn build_unity(all_platforms: bool, csharp: bool, deploy: bool, bolt: bool) -> Result<()> {
...
fn deploy_ffi_to_unity(dylib_path: &str, target: Option<&str>, bolt: bool) -> Result<()> {
    println!("\nDeploying to Unity...");
    ...
    // Also copy the C header — xybrid-ffi only. bolt has no C ABI header (it
    // crosses via DllImport), and Runtime/Native was removed in the bolt
    // migration, so copying here would resurrect it.
    if !bolt {
...
fn build_ffi_android(crate_name: &str, target: &str, release: bool, features: &str) -> Result<()> {
...
fn package_unity_natives(version: &str, output_dir: &Path) -> Result<()> {
    ...
    for (key, plugin_dir, primary_lib) in platforms {
        ...
        let present = if key == "android" {
            // At least one ABI subdir must contain the bolt .so.
            ["arm64-v8a", "armeabi-v7a", "x86_64"]
                .iter()
                .any(|abi| src_dir.join(abi).join("libxybrid_bolt.so").exists())
        } else {
